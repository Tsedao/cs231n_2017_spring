...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        out, aff_relu_cache_1 = affine_relu_forward(X,W1,b1)
        scores, aff_cache_2 = affine_forward(out,W2,b2)
        ...
        dx2, dW2, db2 = affine_backward(dsoft,aff_cache_2)
        dx1, dW1, db1 = affine_relu_backward(dx2,aff_relu_cache_1)
        dW2 += self.reg*W2
        dW1 += self.reg*W1
        grads['W1'] = dW1
        grads['b1'] = db1
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for idx in range(self.num_layers):
        	if idx == 0:
        		dim_in = input_dim
        		dim_out = hidden_dims[idx]
        	elif idx == self.num_layers-1:
        		dim_in = hidden_dims[idx-1]
        		dim_out = num_classes
        	else:
        		dim_in = hidden_dims[idx-1]
        		dim_out = hidden_dims[idx]
        	w_name = 'W%d'%(idx+1)
        	b_name = 'b%d'%(idx+1)
        	self.params[w_name] = np.random.normal(scale=weight_scale,size=(dim_in,dim_out))
        	self.params[b_name] = np.zeros(shape=(dim_out,))

        if self.use_batchnorm:
            for idx in range(self.num_layers-1):
                gamma = 'gamma%d'%(idx+1)
                beta = 'beta%d'%(idx+1)
                self.params[gamma] = np.ones(shape=(hidden_dims[idx],))  # initialize the scale parameter
                self.params[beta] = np.zeros(shape=(hidden_dims[idx],))   # initialize the shift parameter
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        for idx in range(self.num_layers):
            ...
            if idx < self.num_layers-1:
                if self.use_batchnorm:
                    gamma = 'gamma%d'%(idx+1)
                    beta = 'beta%d'%(idx+1)
                    X, cache = affine_bn_relu_forward(X, self.params[w],self.params[b],
        ...
        for idx in range(self.num_layers,0,-1):
            ...
            if idx == self.num_layers:
                ...
                if self.use_batchnorm:
                    gamma = 'gamma%d'%(idx)
                    beta = 'beta%d'%(idx)
                    dx, dw, db, dgamma, dbeta = affine_bn_relu_backward(dx,caches.pop())
                    grads[gamma] = dgamma
                    grads[beta] = dbeta
                else:
